...
| 水系槽单侧公差 | 0.25 mm | 星溪线真机 PASS；待其他 GPX 复验 |
| 水系轮廓 XY 单侧加宽 | 0.15 mm | 星溪线真机 PASS |
| 水系局部承力颈宽 | 1.35 mm | 小于 1.60 mm 轨迹；星溪线真机 PASS |
| 水系局部承力颈高 | 0.55 mm | 星溪线真机 PASS |

`0.25 mm` 是水系单侧间隙，即槽总宽比水件名义宽度大约
`0.50 mm`。该值解决了细长、多弯水系的累计对位误差，用户反馈安装
非常顺利。它已是 Web 工作流的默认经验值，但在至少一条其他 GPX
真机通过前，状态为“候选通用基线”。
...
- 水系公差与轨迹公差必须独立管理，不得回退到共用常量。
- 水系槽必须从加宽/加固后的最终水件几何派生，不得使用原始水系开槽。
