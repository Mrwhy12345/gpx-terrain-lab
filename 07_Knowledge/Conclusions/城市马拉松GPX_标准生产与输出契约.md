...
状态：V7.3.2，继承 V7.3.1 全部质量门，增补西安 V7.3R11 真机经验；不将项目参数泛化为全城默认
修订日期：2026-09-19；V7.2 生效日期：2026-09-09

低 Token 续做入口：[马拉松GPX_低Token生产入口.md](马拉松GPX_低Token生产入口.md)。仅减少重复读取，不降低以下验收要求。

## 1. 目标

用户提供一份马拉松 GPX 后，系统以 TrailPrint3D 为城市空间事实入口，输出可查看、可分色、可拆装、可切片、可追溯的固定尺寸纪念沙盘套装。

生产标准的成效以稳定交付、少返工、可复用和成本可预估衡量，不以版本、文档或检查项数量衡量。
...
主干流程默认保留 TrailPrint3D/OSM 的原始水系尺度与拓扑，不主动拓宽河面。只有通过下述“主水系视觉强化分支”判定，才允许把河面拓宽作为制造派生；该派生不得覆盖或改写事实源。

### 4.1 可选分支：主水系视觉强化

此分支不是城市马拉松的默认步骤。必须同时满足 A、B、C 三组条件：

- **A｜赛事或城市相关性**：该水系与赛道存在明确关系，例如赛道长距离沿河、跨河、起终点邻河；或它是城市识别的核心地理骨架。仅因画面留白或“蓝色不够多”不能触发。
- **B｜100 mm 制造表达失败**：用默认水系完成一次真实尺寸预览或切片后，出现连续主河有效宽度低于约 `1.2 mm`、被离散成碎片、被道路/轨迹完全遮蔽，或指定蓝色无法形成连续可辨刀路中的至少一项。仅凭放大的 Blender 视图“不明显”不能触发。
- **C｜事实基础充分**：原始 GPX 岸别、OSM 水体多边形/中心线和主要桥梁位置已经冻结，能够在强化后做空间回归。数据本身不确定时应先补数据，不能用拓宽掩盖缺失。

分支执行遵循“最小充分强化”：从原始水面逐级增加展示宽度，每轮都生成同尺度顶视或切片证据；一旦河流连续可辨且不压过红色赛道，就立即停止。禁止为了铺满画面继续加宽，也不得新增原数据不存在的支流、跨岸关系或桥梁。

进入分支后必须触发关联要素回归：

1. 道路按新展示水面重新裁切，保留裁切前道路足迹作证据；
2. 普通桥梁与赛道所在桥梁分别恢复，桥宽与两岸道路平滑衔接；
3. 轨迹依据原始 GPX 的岸别和过河位置重新投影、平滑，禁止沿拓宽后河岸错误漂移；
4. 核对水体连通性、河岸顺序、桥梁数量和所有跨河位置，不得产生断头主河、漏桥、假桥或道路落水；
5. 与默认水系版本并排比较，确认水系仍是背景、红色赛道仍是视觉主体。

退出分支的 PASS 条件是：真实尺寸切片中主河具有连续蓝色刀路；桥梁与道路连续；轨迹岸别及过河点与冻结输入一致；没有新增假水系/假桥；视觉层级仍为赛道第一、水系与道路第二。任一项失败即缩小强化幅度或退回默认水系，不得继续下游制造。

桥梁识别无论是否进入该分支，都不得只依赖 `bridge` 标签，应联合检查水体两岸关系和裁切前道路足迹；不得把轨迹投影造成的跨岸线误建成桥。

## 5. 标准输出物

交付执行补充（2026-09-17）：每次向用户交付新版本或优化候选，必须同时链接该版本打印主3MF和“打开即见”的最终装配Blend，禁止只交3MF而遗漏Blend。发送前实际回读Blend并核对最终制造件版本、网格和材料，保存校验报告及SHA-256。候选版也须列出标准清单的已交付/缺失/待验项；未补齐不得声称标准全套完成，FINAL与封板ZIP仍须等真机验收。
...
- `README_先看这里.md`
- `07_QA_打印测试门禁.md`
- `review/PrintRun_XX_打印放行与实测记录.md`：每次真机打印独立建档，失败、取消和复打均不得覆盖
...
制造流水线未直接留下同版本最终 Blend 时，不得重新设计；应从最终 3MF 的真实网格、材料子件和装配变换确定性重建 `.blend`。最终 3MF、重建 Blend、顶/侧/底视图和发布图必须使用同一版本号。模拟实拍图只能在 Blender 三视图 PASS 后生成，图像生成工具不得改变磁孔、底纹、文字、路线、地标或装配结构。

## 5.1 打印放行与 Print Run 身份

每次启动真机打印前，必须从 `07_Knowledge/Templates/马拉松_打印放行与PrintRun记录模板.md` 新建一份不可覆盖的 Print Run 记录。一次打印尝试对应一个 `Run ID`；失败、人工停止、断料、断电或仅打印单件也必须保留原记录，并另开新 Run 复打。

打印放行单必须绑定：精确 3MF 路径、版本与 SHA-256，同版 Blend 或其 SHA-256，Bambu Studio 版本，目标打印机、喷嘴、打印板、工艺预设、打印顺序、层高、支撑设置，耗材语义→槽位→材质/颜色映射，冲刷/擦料塔状态，以及本次 GUI 切片截图路径。CLI 或几何检查只能作为前置证据，不能单独批准开印。

放行前必须在真实 Bambu Studio 中完成“打开→保存→重开→重新切片”，确认无盘外、浮空、缺件、错误支撑、文字丢失、首层底图/磁孔异常及版本残留。只要 3MF 哈希或影响结果的打印机、材料、工艺设置发生变化，原放行立即失效，必须升版本或至少新建 Run 并重新 GUI 检查。

第二次及后续打印必须在开印前列出“本次变更”“冻结不变”和“验证假设”。对比照片应尽量沿用首印的顶视、斜视、底视、文字和装配角度；不得只凭整体观感宣称问题已修复。北京的颜色、道路密度和局部实体化容差仍属项目决定，不由本节泛化为通用默认值。
...
- 设计 Blend 中存在磁孔不代表制造件保留磁孔。磁孔、底纹、承座和文字必须在最终底座 STL 与最终主 3MF 内各回读一次；任一制造分支切换后都重新检查。黑色底座正底视若因低对比看不清孔口，必须补一张浅灰主体、深灰孔底的检查伪色图，不能据“隐约可见”PASS。
...
- 发布图之前须先用最终 3MF/最终 STL 重建同版本 Blender，并固定三个相机语义：顶视方向与装配阅读方向一致；正底视从产品底面向内看且图案保持用户确认的正常阅读方向；斜侧视显示底座厚度、下沉关系和地标高度。镜像、上下颠倒或缺失功能结构均为 FAIL。

### Q5 真机与封板

- 新结构至少完成测试片；首次产品结构完成四件同盘真机抽样。
- 每次打印使用唯一 Run ID；开印前状态必须为 `GUI_PASS`，开始后登记 `PRINT_RUN_ACTIVE`，结束后登记 `PRINT_RUN_COMPLETED` 或失败原因。
- 实测记录包含开始/结束时间、机器占用、成品/冲刷或擦料塔/支撑/失败废料克数、支撑拆除、四件装配结果、缺陷、照片和操作者结论；预计值不得覆盖实测值。
- 第二次及后续打印必须逐项核对本次变更、冻结不变和验证假设；若复打使用不同 3MF 哈希或不同关键切片设置，不能沿用上一 Run 的 GUI_PASS。
- 检查路网清晰度、建筑强度与顶面封闭、轨迹/地标入槽、触觉标识、底座嵌合及全部汉字的实物清晰度。
- 单个零件试印只能验收其声明范围；四件成品的 `PHYSICAL_ACCEPTED` 必须覆盖底座、轨迹、地标、沙盘和装配关系。
- 只有用户接受且 QA 无未披露 P0/P1 问题时才能登记 `PHYSICAL_ACCEPTED` 并复制到 FINAL。
...
`GPX_AUDITED → TRAILPRINT_GENERATED → CREATIVE_CONFIRMED → MANUFACTURING_PASS → 3MF_PASS → GUI_PASS → PRINT_RUN_ACTIVE → PRINT_RUN_COMPLETED → PHYSICAL_ACCEPTED → FINAL_SEALED`

`GUI_PASS` 只对放行单中绑定的 3MF 哈希和打印配置有效；`PRINT_RUN_COMPLETED` 仅表示机器完成，不表示实物合格。任何阶段 FAIL 都必须保留证据并退回相应上游；禁止把失败候选改名为 FINAL。首批至少用三条不同城市全马 GPX 回归，全部通过后，才能宣称“GPX 输入即稳定输出”。

## 9. V7.3.2 标准 checkpoint（后续项目）
...
3. **CP2 TrailPrint3D 原生场景**：保存插件原生 Blend 和参数；审查裁切范围、道路密度、全部主要水系及断头水体，不能只凭好看进入制造层。默认直接冻结原始水系；只有同时满足 4.1 节 A/B/C 条件时，登记并进入“主水系视觉强化”分支，完成关联要素回归后再返回主干。
...
9. **CP8 双重切片 QA 与放行**：自动检查网格、Z、盘内、对象层级、材料和 G-code；随后由真实 Bambu Studio GUI 检查浮空、支撑、料塔、首层底图/磁孔、文字全部顶层刀路以及版本标题。按 5.1 节生成打印放行单，绑定精确 3MF 哈希与配置后才登记 `GUI_PASS`。
10. **CP9 真机、复打与封板**：每次打印建立唯一 Print Run，记录预计/实测耗材、时间、失败件、照片与装配结果；复打先声明变更、冻结项和验证假设。四件实物由用户接受且 QA 关闭 P0/P1 后登记 `PHYSICAL_ACCEPTED`，再生成 SHA-256、发布目录和不可覆盖 FINAL。

太原项目不得直接复制柏林的路线、水系、道路筛选、地标位置、标题缩放或支撑参数；只复用流程、门槛、脚本和工程结构。

## 10. 太原 V7.1 增补

- 历史 GPX 可以在纪念品效率优先的前提下复用，但必须明确“目标届次、输入届次、核对依据和艺术容差”，不得把历史轨迹标成目标年份官方 GPX。
- 路网密度需在过密与过稀之间以同尺寸已发布作品作视觉标尺；保存密/中/疏派生数据，只冻结最终层级，不覆盖 TrailPrint3D 原生源。
- 太原因汾河与赛道关系明确，且默认 100 mm 表达出现窄、碎和蓝色刀路不连续，满足 4.1 节 A/B/C 条件后才进入水系强化分支。该项目结论不能推导为后续城市默认拓宽河流。
- PNG 若已有可信 Alpha，制造工具必须使用 `alpha_only`，不得再次按亮度阈值分解；若输入棋盘格已烘入 RGB，只允许确定性恢复透明度，并比较锁稿与生产轮廓的方向、连通组件和哈希。
- 阶段状态新增 `DIGITAL_COMPLETE / PRINT_TEST_REQUIRED`：表示最终结构、Bambu 原生工程和 GUI 切片已通过，但真机文字、表面、装配、磁孔和成本尚待验证，不等同于 FINAL。

## 11. 柏林 V7R13 真机回归增补

2026-09-18 用户提供正面装配、底面实拍并明确满意，登记 PHYSICAL_ACCEPTED_USER。此状态记录实际成品接受，不追溯覆盖历史 CLI 预设/T 指令问题，也不自动完成成本或完整套装封板。

1. **先连续、后扣槽**：所有道路修复、桥梁恢复、水系派生完成后，以最终安装实体生成轨迹/地标槽。不得再执行会回填槽口的缓冲或全局重网格；若必须执行，重扣槽并重验。连通性审计区分原生独立水体、安装槽造成的合理中断与错误断口。
2. **分材料承托及异常定位**：闭合和整体层截面合并通过，不代表细小材料岛可打印。沙盘单独关闭支撑复切；失败按材料、连通组件分组/二分定位。隐藏根部加深应保持可见顶部不变；删除不可打印的普通建筑残片必须记录位置、尺寸及理由，不能批量删城。建议搭接不是保证值，最终以切片回归为准，不固定复用柏林的根部 Z。
3. **地标保真与可拆支撑**：在用户允许简化时只优化 Tripo 源的脆弱细节，保留轮廓和识别特征。不能为消除警告另造地标；支撑路径、接触处强度与拆除可达性均须检查。局部支撑时间减少不得表述成整件成本同比减少。
4. **真实字形及比例**：按最终字形包络在边框切向、法向两个方向居中，检查外边界和承座禁入区。改变厚度采用分段/分层变换，保留底纹深度、磁孔、槽、公差和可见特征高度；所有装配 Z 同步更新。柏林 8 mm 底座、0.8 mm 下沉、3 mm 露出仅为项目确认方案。
5. **配置缺失即停**：升版必须携带显式输入/配置清单（来源路径或哈希、配色槽、目标机型/喷嘴、工程模板、支撑设置），不得仅复制 STL。缺少七色等模式标记不得静默回退其他槽数；按材料语义映射并验证所有引用。冲刷数组遵循已验证同机型模板结构，禁止套通用长度公式。最终主工程使用精确路径，不用目录中“首个 3MF”。
6. **每次交付可核对**：候选也同时提供主 3MF 与同版 Blend，完整清单的缺失/待验项明示。先前成功截图不能验收新文件。用户满意可以登记真机接受，但精确复打仍需核对实际使用配置，封板仍按 Q5/Q6。
7. **最小上下文**：每个 Job 仅一个短 checkpoint，默认不超过 60 行。长历史先原样归档，再以当前决定替换活动入口；只存历史索引，不默认全文读旧日志。脚本仍依赖的旧文件原位冷归档，未检查依赖前禁止搬移。对话摘要不是逐字聊天导出，也不能清除已载入的聊天上下文。

以上是规范与项目经验；尚未实现的通用自动拦截不得宣称已实现。下一项目优先复用已有工具，在真实需要重复时抽取项目硬编码，避免为整理流程无关重写。

## 12. 西安 V7.3R11 真机回归增补（2026-09-22）

用户确认 R11 实物打印满意并封版。此前候选的可切片、水密或零干涉记录均不追溯改判为真机通过；以下仅固化可迁移的制造和验收方法，继续执行 V7.3.1 的打印放行、Print Run 与封板证据链。

1. **旧底座复用须过语义残影门**：复用已打印底座的尺寸、承座和磁孔时，不能仅替换彩色嵌件。先从最终底面清除前一城市的浅刻、文字、Logo 与槽痕，恢复干净平皮，再按本城已锁定 PNG 重扣图案及磁孔。最终底视必须同时核对「本城应有」和「旧城不得残留」；闭合、体积和干涉 PASS 不能代替语义审查。
2. **轨迹附着须测实际平底**：独立红色轨迹即使水密、正体积且切片无警告，底面若只以若干最低点触床，仍可能在第一层失粘。制造件须回读床面共面底三角、实际接触面积、首层正挤出路径与铺设范围，并在目标机型检查裙边／附着策略；面积或刀路异常即退回重建连续平底。西安 R5 的 `0 mm²` 失败与 R6 的约 `208.4 mm²` 修复只是案例对照，不能作为跨城市面积阈值。轨迹 XY、端点和装配槽在修复后须同源复验。
3. **地标先选承托方案再做槽**：先对最终 Tripo 派生地标判断是完整台基还是实际设计插脚，量化首层面积、上层扩张、槽/底面面积比及抗转定位；不能因插脚能落入宽槽、实体干涉为零，就认定可安装。能保留地标特征且适合浅槽时，优先完整台基同源平底；若必须支撑，除无浮空警告还要核对支撑实际高度、首层覆盖、附着及拆除风险。地标槽须在道路、水系等表面材料完成后，从所有覆盖安装面的实体末次扣除，并用真实旋转和平移复验装配。西安 R7 的高树支撑失败和 R9 的完整台基成功仅证明此件方案，不能一律禁用树支撑。
4. **字形必须经过制造层和切片层双重回归**：同系列三边文字应先复用已真机验证的原生 CJK／Latin 字形生成方法，统一字重与数字风格，不把字体名相同的手工重画字形视为同款。若 0.4 mm 切片吞掉笔画，只加可量化的局部制造余量，再从最终 3MF 对每个汉字、数字和小数点逐层检查有效刀路、字腔、边框居中及净距；整盘切片成功不算文字 PASS。西安使用的 `0.12 mm` 余量不是通用默认值。
5. **底图锁稿与方向不得跳步**：用户批准的是具体 PNG 及其构图，不是可重新创作的文字描述。按「扣图→预处理→制造面镜像→PNG 直接嵌入」留存原图、制造图、哈希、正底视和真实首层刀路；有可靠 Alpha 不再按亮度重分割。切换 A/B 方案必须明确升版并保留旧方案追溯；底面正常阅读方向是最终判据，制造镜像图本身不能代替装配底视。
6. **真机接受后才封版，封版包不得变动**：主 Bambu 3MF、分件补打工程、同版 Blend、制造 STL、获批 PNG 与制造图、预览、QA、成本账本及 SHA-256 清单须统一版本并校验 ZIP。用户的「打印 OK」可登记实物接受，但未提供的装配量测、实耗材、机时和 Token 仍写 `MISSING`；封版后修正须复制升版，不回写 FINAL。西安 R11 的具体尺寸、槽间隙、字帖和图案仅留在 Job，不自动成为新城市默认。
