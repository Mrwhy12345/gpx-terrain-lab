...
状态：V7.3.2，继承 V7.3.1 全部质量门，补充北京真机暴露的“轨迹整线相对高度”与“道路帽层／承托层实体并集”回归；不将项目参数泛化为全城默认
...
- 轨迹高度必须沿主体做空间分位采样，报告相对局部道路／沙盘顶面的最小值、P05、中位数和 P95；起终点浮雕、里程点等局部最高点不得计入主体高度门，更不能用整件 `max Z` 代替整线凸起验收。
- 道路的可见帽层与隐藏承托层必须形成可抵抗 STL 量化、切片公差与真机误差的有效实体重叠，再执行布尔并集并回读为单一正体积实体。报告须记录帽层底、承托层顶、实际重叠和并集后体积；两个各自水密但只共面或临界接触的壳不算通过。
...
8. **真机反例优先于数字假设**：北京 R9 实打证明，轨迹整件最高点通过不等于主体全程凸起，两个子壳分别水密也不等于道路根部可靠封闭。出现实物反例时，先使旧门禁失效并登记照片、文件哈希和打印范围，再复用柏林等真机成功项目的制造关系与验收方法修复；不得复制城市几何。
